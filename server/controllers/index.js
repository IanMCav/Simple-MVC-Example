...
const newDog = (req, res) => {
  ...
  const dogVal = new Dog(dogData);

  const savePromise = newDog.save();

  savePromise.then(() => {
    res.json({ name: dogVal.name, breed: dogVal.breed, age: dogVal.age });
  });
...
const dogByName = (req, res) => {
  ...
  const myDogPromise = new Dog(Dog.findByName(req.query.name, (err, doc) => {
    if (err) {
      return res.json({ err });
    }
    ...
    myDog = new Dog(res.json({ name: doc.name, age: doc.age, breed: doc.breed }));
  ...
  myDogPromise.then(() => {
    ...
    const savePromise = myDog.save();

    savePromise.then(() => res.json({ name: myDog.name, age: myDog.age, breed: myDog.breed }));

    savePromise.catch(err => res.json({ err }));
  });

  myDogPromise.catch(err => res.json({ err }));

  return res.json({ name: myDog.name, age: myDog.age, breed: myDog.breed });
};
